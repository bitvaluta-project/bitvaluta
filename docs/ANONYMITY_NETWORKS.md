# Anonymity Networks with Bitvaluta

Currently only Tor and I2P have been integrated into Bitvaluta. The usage of
...
is made or until `bitvalutad` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `bitvaluta-wallet-cli` and
`bitvaluta-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
Bitvalutad does not support synchronizing the blockchain over onion or I2P hidden services.
...
`--tx-proxy` which tells `bitvalutad` the ip/port of a socks proxy provided by a
...
which tells `bitvalutad` to connect to ".onion" P2P addresses using a socks
...
`--anonymous-inbound`. This option tells `bitvalutad` the inbound address, network
...
which tells `bitvalutad` that a max of 25 inbound Tor connections are being
received at address "5tymba6faziy36md5ffy42vatbjzlye4vyr3gyz6lcvdfximnvwpmwqd.onion:18084" and forwarded to `bitvalutad`
...
forwarded to `bitvalutad` localhost port 18085. Using `tx-proxy`(required), these
...
`bitvalutad` RPC port - which is independent from the configuration for incoming
...
```text
HiddenServiceDir /var/lib/tor/data/bitvaluta-rpc
...
```text
# P2P Hidden service
HiddenServiceDir /var/lib/tor/data/bitvaluta
...
This will store key information in `/var/lib/tor/data/bitvaluta` and `/var/lib/tor/data/bitvaluta-rpc`
and will forward "Tor port" 18084 and 18089 to ports 18084 and 18089 of ip 127.0.0.1, respectively. The file
`/usr/lib/tor/data/bitvaluta/hostname` will contain the ".onion" address for use with `--anonymous-inbound`, and
`/var/lib/tor/data/bitvaluta-rpc/hostname` will contain the ".onion" address for use with RPC.

I2P must be configured with a standard server tunnel. Configuration differs by
I2P implementation.  
You can find guides for i2pd [here](https://docs.getbitvaluta.org/running-node/bitvalutad-tori2p/#__tabbed_1_2).
...
system clock is fairly accurate - many peers on the Bitvaluta network should have
similar timestamps.
...
### Intermittent Bitvaluta Syncing

If a user only runs `bitvalutad` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `bitvalutad` as often as possible to conceal when transactions are being sent.
...
`bitvalutad` currently selects two outgoing connections every 5 minutes for
...
`bitvalutad` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `bitvalutad` should likely disconnect
