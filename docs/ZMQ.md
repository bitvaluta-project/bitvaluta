# The Current/Future Status of ZMQ in Bitvaluta

## ZMQ Pub/Sub

Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Bitvaluta
...
matching supported by both Bitvaluta and ZMQ. The `format`, `context` and `event`
...
The Bitvaluta daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Bitvaluta daemon also ensures that
...
The Bitvaluta daemon will send a `txpool_add` pub exactly once for each
