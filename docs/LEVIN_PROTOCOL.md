...
used by Bitvaluta. The protocol is largely inherited from cryptonote, but has
undergone some changes.

This document also may differ from the `struct bucket_head2` in Bitvaluta's
...
> This document does not currently list all data being sent by the bitvaluta
> protocol, that portion is a work-in-progress. Please take the time to do it
> if interested in learning about Bitvaluta p2p traffic!

## Header

This header is sent for every Bitvaluta p2p message.
...
Bitvaluta when the link is not encrypted. SSL has been proposed as a means to
...
An unsigned 32-bit little endian integer representing the Bitvaluta specific
...
Requests are the basis of the admin protocol for Bitvaluta. The `Q` bit must be
