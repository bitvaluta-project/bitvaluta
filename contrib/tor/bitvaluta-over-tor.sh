...
for dir in \
  ...
  "$DIR/../../build/Windows/master/release/bin"
do
  if test -x "$dir/bitvalutad"
  then
    bitvalutad="$dir/bitvalutad"
    break
  fi
done
if test -z "$bitvalutad"
then
  echo "bitvalutad not found"
  exit 1
fi
echo "Found: $bitvalutad"

TORDIR="$DIR/bitvaluta-over-tor"
TORRC="$TORDIR/torrc"
...
if test "$ready" = 0
then
  echo "Error starting bitvalutad"
  tail -n 400 "$HOME/.bitvaluta/bitvaluta.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*Bitvaluta\ \'" | tail -n 20
  exit 1
fi
