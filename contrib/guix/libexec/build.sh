...
# Copyright (c) 2022-2024 The Bitvaluta Project
...
DISTNAME="bitvaluta-${HOST}-${VERSION}"
...
# target, so we don't spend time building Bitvaluta binaries.
...
GIT_ARCHIVE="${DIST_ARCHIVE_BASE}/bitvaluta-source-${VERSION}.tar.gz"
...
if [ ! -e "$GIT_ARCHIVE" ]; then
    ...
    | sort \
    | tar --create --transform "s,^,bitvaluta-source-${VERSION}/," --mode='u+rw,go+r-w,a+X' --files-from=- \
    | gzip -9n > ${GIT_ARCHIVE}
...
(
    ...
    # Setup the directory where our Bitvaluta build for HOST will be
