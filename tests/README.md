...
Core tests take longer than any other Bitvaluta tests, due to the high amount of computational work involved in validating core components.
...
To run only Bitvaluta's core tests (after building):
...
To run only Bitvaluta's crypto tests (after building):
...
Open the wallet file with `bitvaluta-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
Bitvaluta is integrated into [OSS-Fuzz](https://github.com/google/oss-fuzz) and the project integration
is available [here](https://github.com/google/oss-fuzz/tree/master/projects/bitvaluta). OSS-Fuzz builds
and runs the fuzzers continuously, so long as Bitvaluta's OSS-Fuzz [build script](https://github.com/google/oss-fuzz/blob/master/projects/bitvaluta/build.sh) builds them.

Issues found by OSS-Fuzz are publicly available (following a disclosure deadline) on the OSS-Fuzz issue tracker [here](https://issues.oss-fuzz.com/issues?q=project%3Dbitvaluta).
The issue tracker only displays limited information, and only maintainers with emails listed in the [project.yaml](https://github.com/google/oss-fuzz/blob/master/projects/bitvaluta/project.yaml) have access to full details.

Coverage reports are built on a daily basis and data about this can be found at [introspector.oss-fuzz.com](https://introspector.oss-fuzz.com) [here](https://introspector.oss-fuzz.com/project-profile?project=bitvaluta).

### Build and run fuzzers by way of OSS-Fuzz

**Building Bitvaluta's fuzzers with OSS-Fuzz**

```sh
$ git clone https://github.com/google/oss-fuzz
$ cd oss-fuzz
$ python3 infra/helper.py build_fuzzers bitvaluta

# Display what was build
$ ls build/out/bitvaluta/
...
```sh
$ python3 infra/helper.py run_fuzzer bitvaluta base58_fuzz_tests
...
...
...
To run only Bitvaluta's hash tests (after building):
...
To run only Bitvaluta's performance tests (after building):
...
To run only Bitvaluta's unit tests (after building):
